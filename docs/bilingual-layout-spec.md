...
- Response script uses a foreground-response deadline strategy. Beta1 tests up to 12 chunks translated in parallel to preserve seek coverage, while beta2 tests a two-concurrent-request queue for unstable Google Translate routes.
...
- Compare beta1 12-parallel-chunk strategy against beta2 two-concurrent-request queue before promoting either to stable.
